...
import { VacancyShort, VacancyFull } from "../types/types"
...
export async function searchJobs(query: string, page: number): Promise<SearchResultDTO> {
  ...
  const promise = enqueue(async () => {
    const res = await fetchRetry(`https://api.hh.ru/vacancies?text=${encodeURIComponent(query)}&per_page=100&page=${page}`)
